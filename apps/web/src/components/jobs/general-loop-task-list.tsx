import { generalLoopTaskSchema } from '@market-ops/shared';
...
type GeneralLoopTaskGroup = {
  productId: number;
  productName: string | null;
  imgUrl: string | null;
  tasks: GeneralLoopTask[];
};

const statusLabel: Record<GeneralLoopTask['status'], string> = {
  PENDING: '1차 대기',
  WAITING_REQ2: '2차 진행',
  SUCCEEDED: '완료',
  FAILED: '실패',
...
function groupTasksByProductId(tasks: GeneralLoopTask[]): GeneralLoopTaskGroup[] {
  const groups = new Map<number, GeneralLoopTaskGroup>();
...
export function GeneralLoopTaskList({
  tasks,
  ...
  if (tasks.length === 0) {
    return (
      <div className="rounded-3xl border border-dashed border-subtle bg-bg-card/40 p-6 text-sm text-text-secondary">
        아직 등록된 일반 루프 작업이 없습니다.
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-3">
      {groupedTasks.map((group) => {
        ...
        return (
          <article key={`general-product-${group.productId}`} className="rounded-3xl border border-subtle bg-bg-card/70 p-5">
                ...
                <div key={task.id} className="rounded-2xl border border-subtle bg-bg-card/80 p-4">
                  ...
                  {editableQuantities ? (
                    <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
                      <TaskQuantityEditor kind="general-loop" task={task} running={running} />
                      <div className="flex justify-end">
                        <MacroTaskDeleteButton kind="general-loop" id={task.id} disabled={running} />
                      </div>
                    </div>
                  ) : null}
                </div>
